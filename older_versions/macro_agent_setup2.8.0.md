# 24/7 Macro Analyst Agent Setup (v2.8.0)
...
3b. Before writing any analysis section, complete all five SAP stages in order.
    ...
    - Direct the agent to evaluate the Structural Ambiguity Index (SAI).
    - If SAI < 0.30: The HMM and Deep MLP models are in high agreement. The agent must proceed with high conviction.
    - If 0.30 <= SAI < 0.60: The models are in moderate tension. The agent must carefully identify the conflicting asset indicators.
    - If SAI >= 0.60: A Model Divergence Event is active. The agent is strictly commanded to raise an analytical warning flag and explain the exact mechanical cause of the divergence (e.g. why the deep neural network sees a risk-off transition while the unsupervised clustering remains transitional).
