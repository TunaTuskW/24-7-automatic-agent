# 24/7 Macro Analyst Agent Setup (v2.7.0)
...
3b. Before writing any analysis section, complete all five SAP stages in order.
    ...
    - Cross-verify the unsupervised HMM Bayesian state with the supervised MLP Deep Classifier state. If both states agree, the current market signal is considered highly confirmed. If they diverge, the agent must flag a "Model Divergence Event" and analyze the mathematical cause of the discrepancy in its narrative.
